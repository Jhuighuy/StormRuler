...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      double precision x,y,cxx,cyy,cx,cy,ce,c1,c2,c3,c4,c5,alfa,gbdy
      common/imud2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fmud2/xa,xb,yc,yd,tolmax,relmax
...
c
      implicit none
      integer nx,ny,nxa
      double precision cof(nx,ny,6),beta(nx,nx,*),alfa(nx,nx,*)
      integer index(nx,ny)
...
c
      ...
      double precision beta(nx,nx,*),alfa(nx,nx,*)
c     forward sweep
...
c
      implicit none
      integer nx,ny,i,j,l
      double precision phi(0:nx+1,0:ny+1),frhs(nx,ny),alfa(nx,nx,*),sum
      do j=1,ny
	do i=1,nx
	  phi(i,j)=frhs(i,j)
	end do
...
c
      implicit none
      integer nx,ny,index(nx,ny)
      double precision phi(0:nx+1,0:ny+1),cof(nx,ny,6),beta(nx,nx,*)
      integer iz,j,jb,i
      iz = 0
	...
	call sgsl(beta(1,1,j),nx ,nx ,index(1,j),phi(1,j),iz)
      ...
      double precision cof(nx,ny,6),alfa(nx,nx,*),beta(nx,nx,*)
      double precision dmat(nx,nx,*),zmat(nx,nx,*),sum
      jcur = 1
c
c     set dmat(1)=alfa(1)
c
...
c
      ...
      double precision phi(0:nx+1,0:ny+1),cof(nx,ny,6)
      double precision beta(nx,nx,*),alfa(nx,nx,*)
      double precision zmat(nx,nx,*), dmat(nx,nx,*)
c     forward sweep
...
c     backward sweep
      ...
      double precision phi(0:nx+1,0:ny+1),frhs(nx,ny),sum
      double precision alfa(nx,nx,*),zmat(nx,nx,*)
      do j=1,ny-1
	do i=1,nx
	  phi(i,j)=frhs(i,j)
	end do
...
c
      ...
      double precision phi(0:nx+1,0:ny+1),cof(nx,ny,6),beta(nx,nx,ny),dmat(nx,nx,*)
...
c
      ...
      double precision cof(nx,ny,6),beta(nx,nx,*)
      j = jcur
      do i=1,nx
	do l=1,nx
	  beta(i,l,jcur)=0.0
	...
	beta(nx,2,jcur) = cof(nx,j,2)
      ...
      double precision cof(nx,ny,6),alfa(nx,nx,*)
      do i=1,nx
	do l=1,nx
	  alfa(i,l,jcur)=0.0
	end do
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      double precision phi(0:nx+1,0:ny+1),cof(nx,ny,6)
      double precision dlx,dlx2,dlxx,dly,dly2,dlyy
      double precision x,y,cxx,cyy,cx,cy,ce,c1,c2,c3,c4,alfa,gbdy
      common/imud2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fmud2/xa,xb,yc,yd,tolmax,relmax
